...
#define     WFS_CMD_CIM_AUDIT                   (CIM_SERVICE_OFFSET + 83)
...
//CIM自定义清机加钞事件
#define  WFS_SRVE_CIM_EXCHANGING_NOTESPRESENT   	(CIM_SERVICE_OFFSET + 70)
#define  WFS_SRVE_CIM_EXCHANGING_WAITINPUT	        (CIM_SERVICE_OFFSET + 71)
#define  WFS_SRVE_CIM_EXCHANGING_POSHADNOTES	    (CIM_SERVICE_OFFSET + 72)
#define  WFS_SRVE_CIM_EXCHANGING_ACTION            (CIM_SERVICE_OFFSET + 73)

// WFS_SRVE_CIM_EXCHANGING_POSHADNOTES事件参数
#define  WFS_CIM_EXCHANGING_REFUSEDNOTE     (1)
#define  WFS_CIM_EXCHANGING_SKEWEDNOTE     (2)

// WFS_SRVE_CIM_EXCHANGING_ACTION事件参数
#define  WFS_CIM_EXCHANGING_SORTNOTES      (1)
#define  WFS_CIM_EXCHANGING_LOADNOTES      (2)
...
typedef struct _wfs_cim_audit_cu
{
  BOOL bEmptyAll;
  LPSTR lpPhysicalPositionName;
} WFSCIMAUDITCU, *LPWFSCIMAUDITCU;

typedef struct _wfs_cim_audited_phys_cu
{
  LPSTR lpPhysicalPositionName;
  CHAR cUnitId[5];
  ULONG ulRejected;
  ULONG ulCounted;
  USHORT usPStatus;
} WFSCIMAUDITEDPHYSCU, *LPWFSCIMAUDITEDPHYSCU;

typedef struct _wfs_cim_audit
{
  USHORT usNumPhysicalCUs;
  LPWFSCIMAUDITEDPHYSCU * lppCountedPhysCUs;
} WFSCIMAUDIT, *LPWFSCIMAUDIT;
...
typedef struct _wfs_cim_sort_info
{
	USHORT          usNumber;
	CHAR            cUnitID[5];
	CHAR            cCurrencyID[3];
	ULONG           ulValues;
	ULONG           ulCount;
}WFSCIMSORTINFO, *LPWFSCIMSORTINFO;
