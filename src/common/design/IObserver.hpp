...
namespace __N_OBSERVER__
{
    ...
    private:
		INDEX index{ 0 };
        std::mutex mx{};
        std::unordered_map<INDEX, std::shared_ptr<ISubscriber>> subscribers{};
        std::unordered_map<INDEX, LAMBDA<CONTEXT>> lambdas{};

    protected:
    public:
        INDEX subscribe(std::shared_ptr<ISubscriber> subscriber) noexcept
        {
        ...
        INDEX subscribe(ISubscriber* pSubscriber) noexcept
        {
            return this->subscribe(std::make_shared<ISubscriber>(pSubscriber));
        }
        ...
        void notify(const CONTEXT& context) noexcept
        {
		    std::lock_guard<std::mutex>_lock(this->mx);
